...
export const checkIsKetua = async (userId: string): Promise<{ isKetua: boolean; kelas: string | null }> => {
  const user = await prisma.user.findUnique({
    where: { id: userId },
    select: {
      isKetua: true,
...
// Middleware to check if user is ketua
const isKetua = t.middleware(async ({ ctx, next }) => {
  if (!ctx.session?.user) {
    throw new TRPCError({ code: 'UNAUTHORIZED' })
  }

  const { isKetua: userIsKetua, kelas } = await checkIsKetua(ctx.session.user.id)

  if (!userIsKetua || !kelas) {
    throw new TRPCError({
      code: 'FORBIDDEN',
      message: 'Ketua access required'
    })
  }
...
export const ketuaProcedure = t.procedure.use(isAuthenticated).use(isKetua)
