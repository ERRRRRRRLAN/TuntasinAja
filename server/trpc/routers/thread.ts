...
export const threadRouter = createTRPCRouter({
  ...
  // Get today's tasks for one-click dashboard
  getTodayTasks: protectedProcedure.query(async ({ ctx }) => {
    ...
    const whereClause = isAdmin
      ? {
          // Admin sees all threads from today
          date: {
            gte: today,
            lt: tomorrow,
          },
      ...
      : userKelas
      ? {
          // Filter by kelas of thread author AND today's date
          author: {
            kelas: userKelas,
          },
          date: {
      ...
      : {
          // Public sees all threads from today
          date: {
            gte: today,
            lt: tomorrow,
          },
    ...
    // Get completion statuses for all threads and comments
    const threadIds = filteredThreads.map((t) => t.id)
    const allCommentIds = filteredThreads.flatMap((t) => t.comments.map((c) => c.id))

    const threadStatuses = await prisma.userStatus.findMany({
      where: {
        userId: userId,
        threadId: {
          in: threadIds,
    ...
    const commentStatuses = await prisma.userStatus.findMany({
      where: {
        userId: userId,
        commentId: {
          in: allCommentIds,
    ...
    const threadStatusMap = new Map(
      threadStatuses.map((s) => [s.threadId, s.isCompleted])
    )
    const commentStatusMap = new Map(
      commentStatuses.map((s) => [s.commentId, s.isCompleted])
    )

    // Add completion status to threads
    const threadsWithStatus = filteredThreads.map((thread) => {
      const isThreadCompleted = threadStatusMap.get(thread.id) || false
      const completedComments = thread.comments.filter(
        (c) => commentStatusMap.get(c.id) || false
      ).length
      const totalComments = thread.comments.length
      const allCommentsCompleted = totalComments > 0 && completedComments === totalComments
      ...
      return {
        ...
        completedComments,
        totalComments,
        allCommentsCompleted,
      }
    })

    return {
      threads: threadsWithStatus,
      total: threadsWithStatus.length,
    }
  }),
})
