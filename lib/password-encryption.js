 ...
 * @param {string} password - Password plaintext yang akan di-encrypt
 * @returns {string} Encrypted password dalam format: salt:iv:tag:encrypted
 */
 ...
 * @param {string} encryptedPassword - Encrypted password dalam format: salt:iv:tag:encrypted
 * @returns {string} Password plaintext
 */
function decryptPassword(encryptedPassword) {
  ...
  try {
    ...
    throw new Error(`Failed to decrypt password: ${error.message || 'Unknown error'}`)
  }
}

/**
 * Verify jika encrypted password valid
 * @param {string} encryptedPassword - Encrypted password yang akan di-verify
 * @returns {boolean} true jika valid, false jika tidak
 */
function isValidEncryptedPassword(encryptedPassword) {
  if (!encryptedPassword) {
    return false
  }
