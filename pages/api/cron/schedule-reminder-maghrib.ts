...
// Sends schedule reminder notifications at 6 PM (maghrib) - 18:00 WIB
// Setup in cron-job.org: Schedule 0 11 * * * (11:00 UTC = 18:00 WIB)
  ...
  try {
    ...
    for (const [kelas, subjects] of Object.entries(schedulesByKelas)) {
      try {
        ...
        console.log(`[ScheduleReminder-Maghrib] Sent to ${kelas}:`, {
          subjects: subjectsList,
        ...
        console.error(`[ScheduleReminder-Maghrib] Error processing kelas ${kelas}:`, error)
    ...
    return res.status(200).json({
      success: true,
      message: `Schedule reminder notifications sent (Maghrib)`,
      tomorrow: tomorrowFormatted,
      totalSent,
      totalFailed,
    ...
    console.error('Error in schedule-reminder-maghrib cron:', error)
