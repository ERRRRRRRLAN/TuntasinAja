...
export default function FeedPage() {
  ...
  const [threadOpenedFromReminder, setThreadOpenedFromReminder] =
    useState(false);
  ...
  const { data: userData, isLoading: isLoadingUserData } =
    trpc.auth.getUserData.useQuery(undefined, {
      enabled: !!session,
    });
  ...
  const defaultSort: "newest" | "oldest" | "dueDate" =
    (userSettings?.defaultSort as "newest" | "oldest" | "dueDate") || "newest";
  ...
  const { data: classSubjects, isLoading: isLoadingSubjects } =
    trpc.classSubject.getClassSubjects.useQuery(
  ...
  const { isActive: isSubscriptionActive, isExpired: isSubscriptionExpired } =
    useClassSubscription(isAdmin ? undefined : userKelas || undefined);
  ...
  useEffect(() => {
    ...
    if (filterParam) {
      ...
      const subjects = decodedFilter
        .split(",")
        .map((s) => s.trim())
        .filter((s) => s.length > 0);

      console.log("[FeedPage] Filter from URL:", {
        filterParam,
        decodedFilter,
        subjects,
      });
  ...
  const { data: uncompletedData } =
    trpc.userStatus.getUncompletedCount.useQuery(undefined, {
  ...
  useEffect(() => {
    ...
    const hasShownReminder =
      typeof window !== "undefined" &&
      sessionStorage.getItem(reminderShownKey) === "true";
  ...
  const filteredThreads = useMemo(() => {
    ...
    // Advanced filter by subject
    if (advancedFilter.type === "subject" && advancedFilter.subjectValue) {
      const subjectValue = advancedFilter.subjectValue.toUpperCase().trim();
      filtered = filtered.filter((thread) => {
        const titleUpper = thread.title.toUpperCase();
        return titleUpper.includes(subjectValue);
      });
    }
    ...
    // Sort based on advanced filter
    filtered.sort((a, b) => {
      if (advancedFilter.type === "deadline") {
      ...
      } else if (advancedFilter.type === "newest") {
        // Sort by newest first
        return (
          new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
        );
      } else {
        // Default to newest for subject filter
        return (
          new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
        );
      }
      ...
      <div className="container">
        <div className="page-header">
          <div>
            ...
            >
              Klik PR untuk melihat detail • Centang checkbox untuk menandai
              selesai
            </p>
            <div
              style={{
                        ...
                        style={{
                          ...
                          background:
                            currentPage === pageNum
                              ...
                              : "var(--bg-secondary)",
                          color:
                            currentPage === pageNum ? "white" : "var(--text)",
                          border:
                            currentPage === pageNum
        ...
        >
          ...
          {showFeedbackTooltip && (
            ...
            >
              ...
              <div
                style={{ color: "var(--text-light)", fontSize: "0.8125rem" }}
              >
                Berikan saran dan masukan Anda untuk membantu TuntasinAja
                semakin berkembang
              </div>
              {/* Arrow pointing down */}
              <div
          ...
          <button
            ...
            style={{
              ...
              zIndex: 999, // Lower than quickview overlay (1000)
              animation:
                "fadeInUp 0.5s ease-out 0.2s both, pulse 2s ease-in-out infinite 1s",
            }}
            onMouseEnter={(e) => {
